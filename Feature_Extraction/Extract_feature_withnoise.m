function[feature_person,position_features]=Extract_feature_withnoise(signal)
...
%% add 3db noise on ECG

noise_power = var(ecg) / 2;
noise = sqrt(noise_power) * randn(size(ecg)) * 0.3;
ECG = ecg + noise;
...
%% add 3db noise om PPG
ppg1=awgn(ppg,3,'measured');

ppg2=sgolayfilt(ppg1, 5, 9);
ppg3=conv(ppg2,ones(3,1)/3,'same');
fs=125;
PPG=lowpass(ppg3,2, fs);
