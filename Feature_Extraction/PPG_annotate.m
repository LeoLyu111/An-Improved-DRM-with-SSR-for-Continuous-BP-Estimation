function [ footIndex, systolicIndex, notchIndex, dicroticIndex, time, bpwaveform ] = PPG_annotate( inWaveform, inFs, verbose, varargin )
...
% by uncommenting line 315 in the source code.
%% Systolic peak index
...
% setting global parameters    
    ...
    % Arterial Blood Pressure Waveform
    if strcmp(Units, 'mmHg')
        max_index=min(size(systolicIndex,2),size(footIndex,2));
        meanPressure = (bpwaveform(systolicIndex(1,1:max_index)) + bpwaveform(footIndex(1,1:max_index)))./2;
        pulsePressure = (bpwaveform(systolicIndex(1,1:max_index)) - bpwaveform(footIndex(1,1:max_index)));
        footIndex( (meanPressure < 15) | (meanPressure > 125) | (pulsePressure < 10) ) = [];
        systolicIndex( (meanPressure < 15) | (meanPressure > 125) | (pulsePressure < 10) ) = [];
    end
...
function [ footIndex ] = getFootIndex( bpwaveform, waveformDDPlus, zoneOfInterest )
    ...
    footIndex = FixIndex(footIndex, bpwaveform, Down, 20);
    %}
end
