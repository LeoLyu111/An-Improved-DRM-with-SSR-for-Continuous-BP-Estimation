...
def get_filelist(data_source_dir, data_dir):
    ...
    # print(data_source_dir, data_dir)
...
def get_data(filelist):
    ...
    for i in range(len(filelist)):
        data_load = scio.loadmat(filelist[i])['feature_person']  # this is rank-2
        # sample_data = np.expand_dims(sample_data, axis=0)  # this is rank-3
        sample_data = []
        # print(len(data_load))
        for j in range(len(data_load) - 15):
            sample_data.append(data_load[j:j+16])
        sample_data = np.stack(sample_data[:-1], axis=0)
        
        # for j in range(int(len(data_load)/16)):
        #     sample_data = np.concatenate((sample_data, data_load[j:(j+1)*16]), axis=0)
        if i % block_size == 0:
            if i != 0:
...
def preprocess(hparams, data_dir):
    ...
    if hparams.dataset_sheme in ['normal', 'refined']:
        ...
        if not tf.io.gfile.exists(train_data_dir):
            tf.io.gfile.makedirs(train_data_dir)
        eval_data_dir = os.path.join(data_dir, 'eval_data')
        if not tf.io.gfile.exists(eval_data_dir):
            tf.io.gfile.makedirs(eval_data_dir)
        test_data_dir = os.path.join(data_dir, 'test_data')
        if not tf.io.gfile.exists(test_data_dir):
            tf.io.gfile.makedirs(test_data_dir)
        ...
        # load data
        all_filelist_train = get_filelist(data_source_dir, all_data_dir[0:1])
        all_filelist_test = get_filelist(data_source_dir, all_data_dir[1:])
        all_filelist_eval = get_filelist(data_source_dir, all_data_dir[1:])
        ...
        if hparams.src_tgt_feature_normalize_group == 'all':
            ...
            data_mean = np.mean(all_data_train, axis=(0, 1))
            data_std = np.std(all_data_train, axis=(0, 1))
            # data_mean = np.mean(all_data_train, axis=0)
            # data_std = np.std(all_data_train, axis=0)
            if hparams.customized_waveform_normalize == True:
