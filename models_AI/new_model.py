"""the difference between new_model and model is this program allows 0 bidirectional layers"""
...
InferOutputTuple = collections.namedtuple("InferOutputTuple",
                                          ("infer_src2tgt_output", "infer_tgt2src_output", "infer_tgt_ref", "infer_src_ref", "infer_loss", "predict_beat_count",
                                           "batch_size", "src_seq_len", "tgt_seq_len"))

InferBackwardOutputTuple=collections.namedtuple("InferBackwardOutputTuple",
                                          ("infer_src_ref", "infer_src2tgt2src_output"))
...
class Model(object):
    ...
    def build_graph(self, hparams):
        ...
        with tf.compat.v1.variable_scope("network", dtype=self.dtype, reuse=tf.compat.v1.AUTO_REUSE):
            self.top_scope = tf.compat.v1.get_variable_scope()
    ...
    def _build_bi_lstm(self, hparams):
        ...
        elif self.num_bi_layers == 0:
            utils.print_out("  num_bi_layers = 0, not use bidirectional layers")
            bi_lstm = None
            bi_lstm_condition = ('identical', None)
        else:
            raise ValueError("Unknown num bi layers %s!" % self.num_bi_layers)
        return bi_lstm, bi_lstm_condition

    def _build_uni_lstm(self, hparams):
        if self.num_uni_layers >= 1:
        ...
        elif self.num_uni_layers == 0:
            utils.print_out("  num_bi_layers = 0, not use bidirectional layers")
            uni_lstm = None
            uni_lstm_condition = ('identical', None)
        elif (self.num_bi_layers != 0 and self.num_uni_layers == 0):
            raise ValueError("cannot have no uni layers after bi layers")
        else:
            raise ValueError("Unknown num bi layers %s!" % self.num_uni_layers)
        return uni_lstm, uni_lstm_condition

    def _build_projector(self, hparams, field=None):
        assert field
        if field == "src":
            if hparams.projector_type == "linear":
                projector = tf.compat.v1.layers.Dense(self.src_feature_size, use_bias=True)
            elif hparams.projector_type == "sigmoid":
                projector = tf.compat.v1.layers.Dense(self.src_feature_size, use_bias=True, activation=tf.math.sigmoid)
        elif field == "tgt":
            if hparams.projector_type == "linear":
                projector = tf.compat.v1.layers.Dense(self.tgt_feature_size, use_bias=True)
            elif hparams.projector_type == "sigmoid":
                projector = tf.compat.v1.layers.Dense(self.tgt_feature_size, use_bias=True, activation=tf.math.sigmoid)
        else:
    ...
    def _set_input_ref(self, direction, lstm_input_given, ref_given, seq_len_given):
        if direction == "src2tgt":
            ...
            lstm_input = self.iterator.target_ref
            ref = self.iterator.source_ref
            seq_len = self.iterator.source_sequence_length
        elif direction == "src2src":
            lstm_input = self.iterator.source_ref
            ref = self.iterator.source_ref
            seq_len = self.iterator.source_sequence_length
        elif direction == "tgt2tgt":
            lstm_input = self.iterator.target_ref
            ref = self.iterator.target_ref
            seq_len = self.iterator.target_sequence_length
        elif direction == "src2tgt2src" or direction == "tgt2src2tgt":
            lstm_input = lstm_input_given
            ref = ref_given
            seq_len = seq_len_given
    ...
    def _set_lstm_projector(self, direction):
        ...
        if direction in ["src2tgt", "src2src", "tgt2src2tgt"]:
            ...
            lstm_scope = "src_lstm"
        if direction in ["tgt2src", "tgt2tgt", "src2tgt2src"]:
            bi_lstm, bi_lstm_condition = self.tgt_bi_lstm, self.tgt_bi_lstm_condition
            uni_lstm, uni_lstm_condition = self.tgt_uni_lstm, self.tgt_uni_lstm_condition
            lstm_scope = "tgt_lstm"

        lstm_list = [bi_lstm, uni_lstm]  # bi lstm then stacked with uni lstm
        lstm_condition_list = [bi_lstm_condition, uni_lstm_condition]

        if direction in ["tgt2src", "src2src", "src2tgt2src"]:
            projector = self.src_projector
            projector_scope = "src_projector"
        if direction in ["src2tgt", "tgt2tgt", "tgt2src2tgt"]:
    ...
    def _encode(self, lstm_scope, input, seq_len, lstm_list, lstm_condition_list):
        ...
        lstm_outputs = None
        with tf.compat.v1.variable_scope(lstm_scope):
            for i in range(len(lstm_list)):
                ...
                if lstm_type == "uni":
                    ...
                    bi_outputs, bi_lstm_state = tf.compat.v1.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, input,
                                                                                dtype=self.dtype,
                ...
                elif lstm_type == "identical":
                    # in this case, no lstm is used, just pass the data to next layer
                    lstm_outputs = input
                    lstm_state = None
                else:
    ...
    def compute_loss(self, hparams, direction, lstm_input_given, ref_given, seq_len_given, feature_size):
        ...
        # print(tf.shape(output), tf.shape(lstm_input))
        loss = tf.sqrt(
            tf.reduce_sum(tf.square(output - ref)) / (tf.compat.v1.to_double(self.batch_size * hparams.src_len * feature_size)))
        return loss, output

    def forward(self, hparams, directions=None):
        src2tgt_loss = tgt2src_loss = src2src_loss = tgt2tgt_loss = 0
        src2tgt_output = tgt2src_output = src2src_output = tgt2tgt_output = None
        ...
        if "src2tgt" in directions:
            src2tgt_loss, src2tgt_output = self.compute_loss(hparams, direction="src2tgt", lstm_input_given=None,
                                                             ...
                                                             feature_size=self.tgt_feature_size)
        if "tgt2src" in directions:
            tgt2src_loss, tgt2src_output = self.compute_loss(hparams, direction="tgt2src", lstm_input_given=None,
                                                             ref_given=None, seq_len_given=None,
                                                             feature_size=self.src_feature_size)
        if "src2src" in directions:
            src2src_loss, src2src_output = self.compute_loss(hparams, direction="src2src", lstm_input_given=None,
                                                             ref_given=None, seq_len_given=None,
                                                             feature_size=self.src_feature_size)
        if "tgt2tgt" in directions:
            tgt2tgt_loss, tgt2tgt_output = self.compute_loss(hparams, direction="tgt2tgt", lstm_input_given=None,
                                                             ref_given=None, seq_len_given=None,
                                                             feature_size=self.tgt_feature_size)
        forward_loss = src2tgt_loss + tgt2src_loss + src2src_loss + tgt2tgt_loss
        return forward_loss, src2tgt_output, tgt2src_output

    def backward(self, hparams, directions, src2tgt_output, tgt2src_output, src_ref, tgt_ref, src_seq_len, tgt_seq_len):
        src2tgt2src_loss = tgt2src2tgt_loss = 0
        ## backward directions just include ["src2tgt2src", "tgt2src2tgt"] at most
        if "src2tgt2src" in directions:
            lstm_input = src2tgt_output
            ref = src_ref
            seq_len = src_seq_len
            src2tgt2src_loss, src2tgt2src_output = \
                self.compute_loss(hparams, direction="src2tgt2src", lstm_input_given=lstm_input, ref_given=ref,
                                  seq_len_given=seq_len, feature_size=self.src_feature_size)
        if "tgt2src2tgt" in directions:
            lstm_input = tgt2src_output
            ref = tgt_ref
            seq_len = tgt_seq_len
            tgt2src2tgt_loss, tgt2src2tgt_output = \
                self.compute_loss(hparams, direction="tgt2src2tgt", lstm_input_given=lstm_input, ref_given=ref,
                                  seq_len_given=seq_len, feature_size=self.tgt_feature_size)

        backward_loss = src2tgt2src_loss + tgt2src2tgt_loss
        return backward_loss, src2tgt2src_output, tgt2src2tgt_output
    ...
    def set_mode_phase(self, hparams):
        ...
        if self.mode == tf.estimator.ModeKeys.TRAIN:
            ...
            # Summary
            # self.forward_train_summary = tf.compat.v1.summary.merge(
            ...
            #      tf.summary.scalar("forward_clipped_gradient", tf.compat.v1.global_norm(forward_clipped_grads))])
            "---------------"

            # backward phase
            self.src_ref_given = tf.compat.v1.placeholder(self.dtype, shape=(self.batch_size, self.length, self.src_feature_size))
            self.tgt_ref_given = tf.compat.v1.placeholder(self.dtype, shape=(self.batch_size, self.length, self.tgt_feature_size))
            self.src2tgt_output_given = tf.compat.v1.placeholder(self.dtype,
                                                       shape=(self.batch_size, self.length, self.tgt_feature_size))
            self.tgt2src_output_given = tf.compat.v1.placeholder(self.dtype,
            ...
            """
            self.src_seq_len_given = tf.compat.v1.placeholder(tf.int32, shape=(self.batch_size))
            self.tgt_seq_len_given = tf.compat.v1.placeholder(tf.int32, shape=(self.batch_size))
            ...
            # Gradients
            params = tf.compat.v1.trainable_variables()
            backward_gradients = tf.compat.v1.gradients(self.train_backward_loss, params)
            backward_clipped_grads, self.backward_grad_norm = tf.compat.v1.clip_by_global_norm(backward_gradients, 5.0)
            ...
            self.infer_loss, self.infer_src2tgt_output, self.infer_tgt2src_output = self.forward(hparams, directions=["src2tgt", "tgt2src"])
            # backward phase
            self.src_ref_given = tf.compat.v1.placeholder(self.dtype, shape=(None, self.length, self.src_feature_size))
            self.tgt_ref_given = tf.compat.v1.placeholder(self.dtype, shape=(None, self.length, self.tgt_feature_size))
            self.src2tgt_output_given = tf.compat.v1.placeholder(self.dtype,
                                                       shape=(None, self.length, self.tgt_feature_size))
            self.tgt2src_output_given = tf.compat.v1.placeholder(self.dtype,
                                                       shape=(None, self.length, self.src_feature_size))
            self.src_seq_len_given = tf.compat.v1.placeholder(tf.int32, shape=(None))
            self.tgt_seq_len_given = tf.compat.v1.placeholder(tf.int32, shape=(None))

            _, self.src2tgt2src_output, self.tgt2src2tgt_output = self.backward(
                hparams, directions=self.backward_directions,
                src2tgt_output=self.src2tgt_output_given, tgt2src_output=self.tgt2src_output_given,
                src_ref=self.src_ref_given, tgt_ref=self.tgt_ref_given,
                src_seq_len=self.src_seq_len_given, tgt_seq_len=self.tgt_seq_len_given)
    ...
    def increase_epoch_num(self, sess):
        ...
        return sess.run((self.epoch_num+1))
    ...
    def train_forward(self, sess):
        ...
        output_tuple = TrainForwardOutputTuple(
                                               ...
                                               train_tgt2src_output=self.tgt2src_output,
    ...
    def train_backward(self, sess, input):
        ...
        output_tuple = TrainBackwardOutputTuple(
                                                ...
                                                train_src2tgt2src_output=self.src2tgt2src_output,
                                                train_tgt2src2tgt_output=self.tgt2src2tgt_output)
    ...
    def infer(self, sess):
        ...
        output_tuple = InferOutputTuple(infer_src2tgt_output=self.infer_src2tgt_output,
                                        infer_tgt2src_output=self.infer_tgt2src_output,
                                        infer_tgt_ref=self.iterator.target_ref,
                                        infer_src_ref=self.iterator.source_ref,
    ...
    def infer_backward(self, sess, input):
        assert self.mode == tf.estimator.ModeKeys.PREDICT
        src_ref, tgt_ref, src2tgt_output, tgt2src_output, src_seq_len, tgt_seq_len = input
        feed_dict = {self.src_ref_given: src_ref, self.tgt_ref_given: tgt_ref,
                     self.src2tgt_output_given: src2tgt_output, self.tgt2src_output_given: tgt2src_output,
                     self.src_seq_len_given: src_seq_len, self.tgt_seq_len_given: tgt_seq_len}
        output_tuple = InferBackwardOutputTuple(infer_src_ref=self.src_ref_given,
                                                infer_src2tgt2src_output=self.src2tgt2src_output)
