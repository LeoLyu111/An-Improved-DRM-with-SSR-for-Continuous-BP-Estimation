...
def get_iterator(src_dataset, tgt_dataset, batch_size, random_seed, is_train):
    ...
    src_tgt_dataset = tf.compat.v1.data.Dataset.zip((src_dataset, tgt_dataset))
...
def load_data_mean_std_soft_min_max(hparams, data_dir):
    if hparams.src_tgt_feature_normalize_group == "all":
        ...
        data_soft_min = scio.loadmat(os.path.join(data_dir, 'all_data_soft_min'))['data']
        data_soft_max = scio.loadmat(os.path.join(data_dir, 'all_data_soft_max'))['data']
        ...
        data_soft_min = scio.loadmat(os.path.join(data_dir, 'train_data_soft_min'))['data']
        data_soft_max = scio.loadmat(os.path.join(data_dir, 'train_data_soft_max'))['data']
    return data_mean, data_std, data_soft_min, data_soft_max
...
def create_train_model(model_creator, hparams, data_dir):
    ...
    print('nmb:',data_dir, train_data_path)
    ...
    with graph.as_default(), tf.compat.v1.container("train"):
        ...
        model = model_creator(hparams, iterator=iterator, mode=tf.estimator.ModeKeys.TRAIN)
...
def create_eval_model(model_creator, hparams, data_dir):
    ...
    data_mean, data_std, data_soft_min, data_soft_max = load_data_mean_std_soft_min_max(hparams, data_dir)
    batch_size = eval_data.shape[0]
    graph = tf.Graph()

    with graph.as_default(), tf.compat.v1.container("eval"):
        ...
        model = model_creator(hparams, iterator=iterator, mode=tf.estimator.ModeKeys.EVAL)
...
def create_infer_model(model_creator, hparams, infer_data, batch_size):
    ...
    with graph.as_default(), tf.compat.v1.container("infer"):
        ...
        iterator = get_iterator(src_dataset, tgt_dataset, batch_size=batch_size,
                                random_seed=hparams.random_seed, is_train=False)
        model = model_creator(hparams, iterator=iterator, mode=tf.estimator.ModeKeys.PREDICT)
...
def train_eval(hparams, data_dir, model_dir, log_dir):
    ...
    config_proto = tf.compat.v1.ConfigProto(log_device_placement=False, allow_soft_placement=True)
    config_proto.gpu_options.allow_growth = True
    train_sess = tf.compat.v1.Session(config=config_proto, graph=train_model.graph)
    eval_sess = tf.compat.v1.Session(config=config_proto, graph=eval_model.graph)

    # Summary writer
    # summary_writer = tf.compat.v1.summary.FileWriter(os.path.join(log_dir, "train_log"), train_model.graph)
    # so we really nedd this one?
    ...
    while global_step < hparams.num_train_steps:
        ...
        try:
            ...
            eval(eval_model, eval_sess, model_dir, hparams, log_f)
            train_sess.run(train_model.iterator.initializer)
            continue

        # Process step_result, accumulate stats, and write summary
        global_step = step_result.global_step
        if global_step % hparams.steps_per_test == 0:
            train_model.model.saver.save(train_sess, os.path.join(model_dir, "model.ckpt"), global_step=global_step)
            # return global_step
        ...
        if global_step % hparams.steps_per_stats == 0:
            ...
            utils.print_out(
                ...
                (
                    ...
                    time.ctime()), log_f)
            # for key in info:
            #     summary_writer.add_summary(tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=key, simple_value=info[key])]),
            #                                global_step)
...
def eval(eval_model, eval_sess, model_dir, hparams, log_f):
    with eval_model.graph.as_default():
        ...
        while True:
            try:
                ...
                #         tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=key, simple_value=eval_info[key])]),
                #         global_step)
                break
...
def infer(hparams, data_dir, model_dir, result_dir):
    ...
    for j in range(len(infer_data_path)):
        ...
        config_proto = tf.compat.v1.ConfigProto(log_device_placement=False, allow_soft_placement=True)
        config_proto.gpu_options.allow_growth = True
        infer_sess = tf.compat.v1.Session(config=config_proto, graph=infer_model.graph)
        with infer_model.graph.as_default():
            ...
            forward_result = infer_model.model.infer(infer_sess)
            infer_avg_beat_loss = forward_result.infer_loss
            total_predict_beat_count = forward_result.predict_beat_count
            total_sequence_count = forward_result.batch_size
            src_ref, tgt_ref, src2tgt_output, tgt2src_output, src_seq_len, tgt_seq_len = (
                forward_result.infer_src_ref, forward_result.infer_tgt_ref,
                forward_result.infer_src2tgt_output, forward_result.infer_tgt2src_output,
                forward_result.src_seq_len, forward_result.tgt_seq_len)
            backward_input = [src_ref, tgt_ref, src2tgt_output, tgt2src_output, src_seq_len, tgt_seq_len]
            backward_result=infer_model.model.infer_backward(infer_sess, backward_input)
            # scale back to the original size
            # (both the infered one and the input one needed to be scaled back, because they are both normalized before)
            origin_tgt_data = infer_data[:, :,
                              hparams.src_feature_size: hparams.src_feature_size + hparams.tgt_feature_size]
            infer_tgt_output = forward_result.infer_src2tgt_output
            origin_src_data = infer_data[:, :, 0:hparams.src_feature_size]
            infer_src_output = backward_result.infer_src2tgt2src_output
            origin_norm_src_feature=origin_src_data
            origin_norm_tgt_feature = origin_tgt_data
            infer_norm_src_feature = infer_src_output
            infer_norm_tgt_feature=infer_tgt_output
            origin_real_tgt_feature = np.zeros(origin_tgt_data.shape)
            infer_real_tgt_feature = np.zeros(infer_tgt_output.shape)
            origin_real_src_feature = np.zeros(origin_src_data.shape)
            infer_real_src_feature = np.zeros(infer_src_output.shape)
            if hparams.tgt_feature_normalize_method=='normal':
                for i in range(hparams.tgt_feature_size):
                    origin_real_tgt_feature[:, :, i] = (
                ...
                for i in range(hparams.src_feature_size):
                    origin_real_src_feature[:, :, i] = (
                            origin_src_data[:, :, i] * data_std[:, i]
                            + data_mean[:, i])
                    infer_real_src_feature[:, :, i] = (
                            infer_src_output[:, :, i] * data_std[:, i]
                            + data_mean[:, i])
            elif hparams.tgt_feature_normalize_method=='soft_minmax':
                for i in range(hparams.tgt_feature_size):
                    origin_real_tgt_feature[:, :, i] = (
                            origin_tgt_data[:, :, i] * (data_soft_max[:,hparams.src_feature_size + i]-data_soft_min[:,hparams.src_feature_size + i])
                            + data_soft_min[:, hparams.src_feature_size + i])
                    infer_real_tgt_feature[:, :, i] = (
                            infer_tgt_output[:, :, i] * (data_soft_max[:, hparams.src_feature_size + i]-data_soft_min[:, hparams.src_feature_size + i])
                            + data_soft_min[:, hparams.src_feature_size + i])
                for i in range(hparams.src_feature_size):
                    origin_real_src_feature[:, :, i] = (
                            origin_src_data[:, :, i] * (data_soft_max[:, i]-data_soft_min[:,hparams.i])
                            + data_soft_min[:, i])
                    infer_real_src_feature[:, :, i] = (
                            infer_src_output[:, :, i] * (data_soft_max[:, i]-data_soft_min[:,  i])
                            + data_soft_min[:, i])
            infer_info = {}
            ...
            # save_result()
            step_result_dir = os.path.join(result_dir, 'step-' + str(global_step)+'-epoch-'+str(epoch_num))
            if not tf.io.gfile.exists(step_result_dir):
                tf.io.gfile.makedirs(step_result_dir)
            scio.savemat(os.path.join(step_result_dir, data_name + '_origin_real_tgt.mat'), {'data': origin_real_tgt_feature})
            scio.savemat(os.path.join(step_result_dir, data_name + '_origin_real_src.mat'),
                         {'data': origin_real_src_feature})
            utils.print_out("save origin tgt data to %s" % os.path.join(step_result_dir, data_name + '_origin_real_tgt.mat'))
            utils.print_out(
                "save origin src data to %s" % os.path.join(step_result_dir, data_name + '_origin_real_src.mat'))
            scio.savemat(os.path.join(step_result_dir, data_name + '_prediction_real_tgt.mat'),
                         {'data': infer_real_tgt_feature})
            scio.savemat(os.path.join(step_result_dir, data_name + '_prediction_real_src.mat'),
                         {'data': infer_real_src_feature})
            utils.print_out(
                "save prediction src2tgt data to %s" % os.path.join(step_result_dir, data_name + '_prediction_real_tgt.mat'))
            utils.print_out(
                "save prediction src2tgt2src data to %s" % os.path.join(step_result_dir,
                                                                    data_name + '_prediction_real_src.mat'))
            scio.savemat(os.path.join(step_result_dir, data_name + '_origin_norm_tgt.mat'),
                         {'data': origin_norm_tgt_feature})
            scio.savemat(os.path.join(step_result_dir, data_name + '_origin_norm_src.mat'),
                         {'data': origin_norm_src_feature})
            utils.print_out(
                "save origin tgt data to %s" % os.path.join(step_result_dir, data_name + '_origin_norm_tgt.mat'))
            utils.print_out(
                "save origin src data to %s" % os.path.join(step_result_dir, data_name + '_origin_norm_src.mat'))
            scio.savemat(os.path.join(step_result_dir, data_name + '_prediction_norm_tgt.mat'),
                         {'data': infer_norm_tgt_feature})
            scio.savemat(os.path.join(step_result_dir, data_name + '_prediction_norm_src.mat'),
                         {'data': infer_norm_src_feature})
            utils.print_out(
                "save prediction src2tgt data to %s" % os.path.join(step_result_dir,
                                                                    data_name + '_prediction_norm_tgt.mat'))
            utils.print_out(
                "save prediction src2tgt2src data to %s" % os.path.join(step_result_dir,
                                                                        data_name + '_prediction_norm_src.mat'))
            # if need to remove the outlier from loss in test
            compute_loss_mask=np.ones(origin_real_tgt_feature.shape)
            if hparams.remove_outlier_from_loss_in_test==True:
                for i in range(hparams.tgt_feature_size):
                    for p in range(origin_real_tgt_feature.shape[0]):
                        for q in range(origin_real_tgt_feature.shape[1]):
                            if (origin_real_tgt_feature[p,q,i]>data_soft_max[:,hparams.src_feature_size+i] or
                                origin_real_tgt_feature[p,q, i]<data_soft_min[:,hparams.src_feature_size+i]):
                                compute_loss_mask[p,q,:]=0
                origin_real_tgt_feature=compute_loss_mask*origin_real_tgt_feature
                infer_real_tgt_feature=compute_loss_mask*infer_real_tgt_feature

            # performance metrics calculation
            # RMSE: root mean square error
            real_test_loss_SBP_RMSE = np.sqrt(
                np.mean(np.square(origin_real_tgt_feature[:, :, 0] - infer_real_tgt_feature[:, :, 0])))
            real_test_loss_DBP_RMSE = np.sqrt(
                np.mean(np.square(origin_real_tgt_feature[:, :, 1] - infer_real_tgt_feature[:, :, 1])))
            real_test_loss_MBP_RMSE = np.sqrt(
                np.mean(np.square(origin_real_tgt_feature[:, :, 2] - infer_real_tgt_feature[:, :, 2])))
            # MAD: mean absolute difference
            real_test_loss_SBP_MAD = np.mean(
                np.absolute(origin_real_tgt_feature[:, :, 0] - infer_real_tgt_feature[:, :, 0]))
            real_test_loss_DBP_MAD = np.mean(
                np.absolute(origin_real_tgt_feature[:, :, 1] - infer_real_tgt_feature[:, :, 1]))
            real_test_loss_MBP_MAD = np.mean(
                np.absolute(origin_real_tgt_feature[:, :, 2] - infer_real_tgt_feature[:, :, 2]))
            # MD: mean difference (absolute is not used!)
            real_test_loss_SBP_MD = np.mean(origin_real_tgt_feature[:, :, 0] - infer_real_tgt_feature[:, :, 0])
            real_test_loss_DBP_MD = np.mean(origin_real_tgt_feature[:, :, 1] - infer_real_tgt_feature[:, :, 1])
            real_test_loss_MBP_MD = np.mean(origin_real_tgt_feature[:, :, 2] - infer_real_tgt_feature[:, :, 2])
            # SD1: the standard deviation of delta
            real_test_loss_SBP_SD1 = np.sqrt(
                np.mean(np.square(
                    (origin_real_tgt_feature[:, :, 0] - infer_real_tgt_feature[:, :, 0]) - real_test_loss_SBP_MD)))
            real_test_loss_DBP_SD1 = np.sqrt(
                np.mean(np.square(
                    (origin_real_tgt_feature[:, :, 1] - infer_real_tgt_feature[:, :, 1]) - real_test_loss_DBP_MD)))
            real_test_loss_MBP_SD1 = np.sqrt(
                np.mean(np.square(
                    (origin_real_tgt_feature[:, :, 2] - infer_real_tgt_feature[:, :, 2]) - real_test_loss_MBP_MD)))
            ...
            name_collection = ['real_test_loss_SBP_RMSE',
                               'real_test_loss_SBP_MAD',
                               'real_test_loss_SBP_MD',
                               'real_test_loss_SBP_SD1',
                               'real_test_loss_SBP_SD2', ]
            metric_collection = [real_test_loss_SBP_RMSE,
                                 real_test_loss_SBP_MAD,
                                 real_test_loss_SBP_MD,
                                 real_test_loss_SBP_SD1,
                                 real_test_loss_SBP_SD2, ]
